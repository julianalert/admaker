export const metadata = {
  title: 'My Photoshoots',
  description: 'View and manage your AI product photo photoshoots.',
}

import Link from 'next/link'
import CampaignsPosts from './campaigns-posts'
import CampaignsPagination from './campaigns-pagination'
import { getUserCampaignsPaginated } from './get-campaigns'

const PER_PAGE = 8

export default async function Photoshoot({
  searchParams,
}: {
  searchParams: Promise<{ page?: string }>
}) {
  const { page: pageParam } = await searchParams
  const page = Math.max(1, parseInt(String(pageParam), 10) || 1)
  const { campaigns, totalCount, totalPages, page: currentPage } = await getUserCampaignsPaginated(page, PER_PAGE)
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">

      {/* Page header */}
      <div className="sm:flex sm:justify-between sm:items-center mb-5">

        {/* Left: Title */}
        <div className="mb-4 sm:mb-0">
          <h1 className="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">My Photoshoots</h1>
        </div>

        {/* Right: Actions */}
        <div className="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">

          {/* New photoshoot button */}
          <Link className="btn cursor-pointer bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white" href="/new">
            <svg className="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
              <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
            </svg>
            <span className="max-xs:sr-only">New photoshoot</span>
          </Link>

        </div>

      </div>

      <div className="text-sm text-gray-500 dark:text-gray-400 italic mb-4">{totalCount} Photoshoot{totalCount !== 1 ? 's' : ''}</div>

      {/* Content */}
      <CampaignsPosts campaigns={campaigns} />

      {/* Pagination */}
      <div className="mt-8">
        <CampaignsPagination currentPage={currentPage} totalPages={totalPages} />
      </div>
    </div>
  )
}
